...
export default function MessageInput({
  ...
  return (
    ...
    >
      ...
      {/* Drag overlay */}
      {dragOver && (
        <div className="mx-4 my-3 py-6 flex flex-col items-center gap-2.5 border-2 border-dashed rounded-xl text-accent animate-pulse-border bg-accent/5">
          <div className="w-12 h-12 rounded-full bg-accent/10 flex items-center justify-center animate-gentle-float">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-muted mt-0.5">Images, docs, audio up to 50 MB</p>
          </div>
        </div>
      )}
          ...
          <div className="flex-1 relative min-w-0">
            ...
            {value.length > 500 && (
              <span className={`absolute bottom-1 right-24 text-[11px] pointer-events-none animate-fade-in flex items-center gap-1.5 ${
                ...
                {value.length > 1000 && <span>{value.trim().split(/\s+/).filter(Boolean).length}w</span>}
