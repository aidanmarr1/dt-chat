          ...
          return (
            <button
              ...
              className={`w-full relative overflow-hidden rounded-lg text-left transition-all duration-200 active:scale-[0.99] cursor-pointer ${
                opt.voted
                  ? isOwn ? "ring-2 ring-background/40" : "ring-2 ring-accent/40"
                  : isOwn ? "hover:bg-background/10" : "hover:bg-surface"
              }`}
            >
              {/* Fill bar */}
              <div
                className={`absolute inset-y-0 left-0 rounded-r-sm transition-all duration-500 ease-out ${hasVoted ? "animate-poll-fill" : ""} ${
                  opt.voted
                    ? isOwn ? "bg-background/25 shadow-[inset_0_0_8px_rgba(255,255,255,0.1)]" : "bg-accent/15 shadow-[inset_0_0_8px_rgba(var(--acc-rgb),0.1)]"
